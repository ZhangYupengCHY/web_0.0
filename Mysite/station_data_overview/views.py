...
def overview_days(request):
    ...
    def group_all_days(station_overview_ori):
        ...
        all_days_group = pd.DataFrame(all_days_group,
                                      columns=['day_distance', 'ad_spend', 'ad_sales', 'shop_sales', 'station_num',
                                               'acos',
                                               'sale_rate', 'spend_rate'])
...
def overview_stations(request, choose_day):
    ...
    def group_all_stations(station_overview_ori, choose_day):
        ...
        station_overview_date_after_group['spend_rate'] = [str(round(100 * spend / sale, 2)) + '%' if sale > 0 else '0%'
                                                           for
                                                           spend, sale in
                                                           zip(station_overview_date_after_group['ad_spend'],
...
def upload_files(request):
    return render_to_response('upload_files.html', locals())